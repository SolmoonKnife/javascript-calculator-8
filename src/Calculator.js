export function calculator(inputString) {
    ...
    const match = inputString.match(customSlicerRegex);

    let numbers = [];
    if (match) {
        const customSlicer = match[1]; // ex) ';'
        const numbersPart = inputString.substring(match[0].length); // ex) '//;\n' 이후부터 취한다

        numbers = numbersPart.split(customSlicer).map(Number);
    } else {
        numbers = inputString.split(normalSlicerRegex).map(Number);
    }
